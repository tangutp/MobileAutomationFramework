...
public class LoginTest_Klov {
    ...
    public void InitializeDeviceDriver(String deviceName,String platformName,String app,String URL_,String extentConfigFile,String exportTestSummaryReport,Boolean replaceExistingReport,Boolean captureVideo) throws MalformedURLException, ATUTestRecorderException {
        ...
        //klov = new ExtentXReporter("localhost", 1337);
...
//        extent = new ExtentReports(exportTestSummaryReport,replaceExistingReport,DisplayOrder.NEWEST_FIRST);
//            extent.attachReporter(htmlReporter,klov);

        //klov.loadXMLConfig(new File(extentConfigFile));
    ...
    @Test(groups = "Smoke", dataProvider = "LoginTestData" , dataProviderClass = FrameworkTestData.DataTable.class )

    public void Login(String emailID, String password) throws MalformedURLException {
        test=extent.createTest("Login Validation");
        test.assignCategory("Smoke");
        test.assignAuthor("Pavan");
        LoginPageObjects lpo = new LoginPageObjects(driver);
        lpo.enterEmailID(emailID);
        lpo.enterPassword(password);
        lpo.click();
        lpo.acceptAlert();

         if (lpo.verifyHomePage().equals("Logged into App")) {
            test.log(Status.PASS, "Test Passed");
        }else{
            test.log(Status.FAIL,"Invalid Details");
            lpo.clearEmailID();
            lpo.clearPassword();
        }
    ...
    public void endReport(Boolean captureVideo) throws ATUTestRecorderException {
        ...
        extent.flush();
        klov.flush();
        System.out.println(klov.getReportName());
